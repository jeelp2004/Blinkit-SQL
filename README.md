...
# Dataset Description
The dataset Consists of Table group_stage_team_stats.csv and overall_wc_stats.csv
...
The group_stage_team_stats table offers a comprehensive breakdown of each team’s performance in the FIFA World Cup tournament group stage. Here’s how each column contributes to the analysis:

1.group: Indicates the group the team belongs to, providing context for comparisons within the group.

2.rank: Current ranking within the group, based on ongoing performance, aiding in determining standings.

3.team: The team’s name, used to identify and filter data for each team.

4.matches_played: Total games played, which is essential for calculating average metrics.

5.wins, draws, losses: Provides the number of outcomes, helping calculate win/draw/loss rates.

6.goals_scored: Total goals scored, a primary indicator of the team’s offensive strength.

goals_against: Total goals conceded, used to assess defensive performance.

goal_difference: The net difference between goals scored and goals conceded; a positive value typically suggests strong offensive/defensive balance.

points: Total points based on FIFA’s point system, where wins, draws, and losses directly affect team standings.

expected_goal_scored: Expected goals scored, reflecting offensive effectiveness based on opportunities created.

exp_goal_conceded: Expected goals conceded, indicating defensive resilience against opponents' chances.

exp_goal_difference: The difference between expected goals scored and conceded; positive values suggest the team’s potential to outperform or concede fewer goals than expected.

exp_goal_difference_per_90: Expected goal difference normalized to a 90-minute match. This per-90 metric is helpful for comparing team performance independent of total playtime, providing insights into a team’s efficiency per match duration.
